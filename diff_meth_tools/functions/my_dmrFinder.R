...
my_dmrFinder <- function(bstat, cutoff = NULL, qcutoff = c(0.025, 0.975),
  ...
  if( any(is.na(regions$down)) ){
    b = apply(regions$down, 1, function(x) any(is.na(x)))
    regions$down = regions$down[-which(b),]
    regions$up = regions$up[-which(b),]
  }
  if( any(is.na(regions$up)) ){
    d = apply(regions$up, 1, function(x) any(is.na(x)))
    regions$down = regions$down[-which(d),]
    regions$up = regions$up[-which(d),]
  }
  ##############################
...
# require(R.utils)
# library("bsseq")
# reassignInPackage("dmrFinder", pkgName="bsseq", "my_dmrFinder")
# 
# assignInNamespace('dmrFinder',my_dmrFinder,ns='bsseq')
# 
