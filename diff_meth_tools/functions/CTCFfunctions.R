...
accuracy.atleast1DM = function(methylBase.obj, co,
                               supp.tbl2.gr,
                               pred.dm.per.cell.line,
  ...
  if(length(indx.peakschange.overlp.pred.dm)!=0){
    # peak change + DM
    TP = peaks.change[  indx.peakschange.overlp.pred.dm  ]
    # peak change + no-DM
    FN = peaks.change[-indx.peakschange.overlp.pred.dm  ]
  }else{
    # peak change + DM
    TP = integer(0)
    # peak change + no-DM
  ...
  if(length(indx.peaksnochange.overlp.pred.dm)!=0){
    # no peak change  + DM
    FP = peaks.dont.change[ indx.peaksnochange.overlp.pred.dm  ]
    # no peak change + no-DM
    TN = peaks.dont.change[- indx.peaksnochange.overlp.pred.dm  ]
  }else{
    # no peak change  + DM
    FP = integer(0)
    # no peak change + no-DM
    TN = peaks.dont.change
  }
  TP = length(TP); FN=length(FN); FP=length(FP); TN = length(TN)
